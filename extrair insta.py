...
# APENAS BING (duckddog removido)
BACKENDS = ["bing"]

# =========================================================
# TERMOS (mantido igual, focado em Salvador/BA)
# =========================================================
...
# =========================================================
# FUNÇÕES (ALTERADAS PARA INSTAGRAM)
# =========================================================
...
def limpar_link(url):
    ...
    # Construir URL limpa do Instagram
    url = f"https://www.instagram.com/{username}/"
    url = url.split("?")[0].split("#")[0].rstrip("/")
    return url


def eh_instagram(url):
    dominio = urlparse(url).netloc.lower()
    return dominio in ("instagram.com", "www.instagram.com")


def instagram_util(url):
    if not eh_instagram(url):
        return False
    path = urlparse(url).path.strip("/").lower()
    if not path:
        return False
    # Ignorar páginas não-perfil do Instagram
    bloqueados = {
        "explore", "tags", "reels", "tv", "live", "direct", "messages",
        "notifications", "search", "accounts", "login", "register",
        "about", "help", "press", "api", "jobs", "privacy", "terms",
        "cookie", "language", "settings", "developer", "business",
        "creator", "influencer", "meta", "p", "reel", "story",
    }
    if path in bloqueados or any(path.startswith(b) for b in bloqueados):
        return False
    if len(path) < 1 or len(path) > 30:
        return False
    if path.startswith("-") or path.endswith("-"):
        return False
    # Validar formato de username do Instagram
...
def gerar_consultas():
    ...
    for termo in TERMOS:
        base = [
            f'"{termo}" salvador instagram',
            f'"{termo}" bahia instagram',
            f'"{termo}" instagram',
        ]
        consultas.extend(base)
        for extra in EXTRAS:
            consultas.extend([
                f'"{termo}" "{extra}" instagram',
                f'intitle:"{extra}" "{termo}" instagram',
            ])
    # Adicionar consultas específicas para perfis do Instagram
    for termo in TERMOS[:50]:  # Limitar para não ficar muito grande
        consultas.append(f'site:instagram.com "{termo}" salvador')
        consultas.append(f'site:instagram.com "{termo}" bahia')
...
def buscar_uma_consulta(consulta, backend, vistos, MAX_LINKS):
    ...
    for item in resultados:
        ...
        if not href or not instagram_util(href) or href in vistos:
            continue
        vistos.add(href)
...
def main():
    print("\n========================================")
    print("EXTRATOR INSTAGRAM TURBO - BING")
    print("========================================")
